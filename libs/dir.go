...
func Init() {
	...
	http.Handle("/fonts/", http.StripPrefix("/js/",http.FileServer(http.Dir("assets/fonts"))))
...
func InitTemplates(base string) error {
	...
	markdownTemplate, err = template.ParseFiles(filepath.Join(base, "templates/markdown.tmpl"))
	return err
}
...
var (
	...
	// markdownTemplate holds the front page template.
